...
<div>
		...
		<div class="container white-shadow">
			<div class="row">
				<div class="detail-pic">
					<img :src="'/storage/'+resume.user.profile_picture_url" class="img containFit" alt="" />
				</div>
			...
			<div class="row bottom-mrg">
				<div class="col-md-8 col-sm-8">
					<div class="detail-desc-caption">
						...
						<p class="description-text no-break" v-html="formatText(resume.about)"></p>
					</div>
				</div>
				<div class="col-md-4 col-sm-4 printblock">
					<div class="get-touch">
						<h4>{{lang('resume_general_get_in_touch')}}</h4>
						<ul>
							<li><i class="fa fa-user icon-compact-info"></i><span>{{resume.user.name}} {{resume.user.surname}}</span></li>
							...
							<li v-if="match.accepted_by_user"><i class="fa fa-envelope icon-compact-info"></i><span>{{resume.user.email}}</span></li>
							<li v-if="match.accepted_by_user"><i class="fa fa-envelope icon-compact-info"></i><span>{{resume.user.mobile_number}}</span></li>
							<li><i class="fa fa-eye icon-compact-info"></i><span>{{lang('applicant_dashboard_about_viewed_by')}} {{resume.user.profile_viewed}}</span></li>
			...
			<div class="row no-padd no-print" >
	...
	<section class="full-detail-description full-detail">
		<div class="container">
			...
			<div class="row row-bottom no-print" >
				<h2 class="detail-title">{{lang('public_resume_match_options')}}</h2>
				<p v-if="match.job.job_title">{{lang('public_resume_matched_to_job')}} <strong>{{match.job.job_title.title}}</strong>.</p>
				<div style="text-align: center">
					<div class="match-option-buttons">
						<button class="btn btn-primary accept" :disabled="!!match.accepted_by_company" @click="acceptResume"><i class="fa fa-check"></i> {{match.accepted_by_company===1 ? lang('resume_general_accepted'):lang('resume_general_accept_resume')}}</button>
						<button class="btn btn-primary reject" :disabled="!!match.accepted_by_company" @click="declineResume"><i class="fa fa-times"></i> {{match.accepted_by_company===-1 ? lang('resume_general_rejected'):lang('resume_general_reject_resume')}}</button>
						<button v-if="!match.requested_short_video" :disabled="!!match.accepted_by_company" @click="requestShortVideo" class="btn btn-primary rsv">{{lang('public_resume_request_csv')}}</button>
						<button v-else-if="match.short_video_url" @click="playShortVideo" class="btn btn-primary rsv"><i class="fa fa-play"></i> {{lang('public_resume_view_csv')}}</button>
						<button v-else class="btn btn-primary rsv" disabled>{{ lang('public_resume_csv_requested') }}</button>
					</div>
				</div>
			</div>
			<!-- </editor-fold> -->
			...
			<div class="row row-bottom no-print">
				...
				<div style="text-align: center">
					...
					<!--<iframe width="70%" height="450" src="https://www.youtube.com/embed/9BMwcO6_hyA" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>-->
				</div>
			</div>
			<!-- </editor-fold> -->
			...
			<div class="row row-bottom">
				...
				<ul class="detail-list">
					<li v-for="license in resume.driver_licenses" class="matchedCr">{{license.name}}</li>
				</ul>
			</div>
			<!-- </editor-fold> -->
...
export default {
	...
	created() {
		this.$store.dispatch("fetchMatchPreview", {
			resume_id: this.$route.params.resume_id,
			job_id: this.$route.params.job_id
		}).then(res => {
			this.match = res.data.result;
		})
		.finally(() => {
			this.loading_resume = false;
		})
	},
	mounted() {
		this.initMap();
	...
	methods: {
		...
		acceptResume() {
			this.$store.dispatch("acceptResume", {
				job_id: this.match.job.id,
				resume_id: this.match.resume.id
			}).then(res => {
				if(res.data.success) {
					this.match.accepted_by_company = 1;
				} else {
		...
		requestShortVideo() {
			...
			this.$root.$prompt.open({
					...
					this.$store.dispatch("requestShortVideo", {
						...
						if(res.data.success) {
							this.match.requested_short_video = 1;
						} else {
							this.$root.$alert.open(res.data.error);
						}
					})
		...
		playShortVideo() {
			// window.open('/storage/'+this.match.short_video_url, "_blank").focus();
			this.$root.$video.open('CSV Video','/storage/'+this.match.short_video_url);
		},
		openDoc(doc) {
	...
	computed: {
		resume() {
			return this.match.resume;
		},
		age() {
			// return moment().diff(moment(this.resume.user.birthday, "YYYY-DD-MM"), "years");
			return this.calculateAge(this.resume.user.birthday);
		},
		...
		userLocation() {
			if(this.resume.user && this.resume.user.city && this.resume.user.city.country) {
				return this.resume.user.city.title+", "+this.resume.user.city.country.title;
			}
			return "";
		},
		videoUrl() {
