		...
		<div class="mail-card unread" v-for="alert in alerts">
			<header class="card-header cursor-pointer collapsed" data-toggle="collapse" data-target="#full-message" aria-expanded="false">
				<div class="card-title flexbox"  style="width: 100%">
					<img style="object-fit: contain" v-if="alerts.user" class="img-responsive img-circle avatar" :src="'/storage/'+alert.user.profile_picture_url" alt="...">
					<div style="width: 100%">
						...
						<div class="col-md-3">
							<small v-if="alert.url"><router-link tag="a" :to="alert.url" class="text-info" >{{lang('company_dashboard_alerts_view_detail')}}</router-link></small>
...
export default {
	...
	methods: {
		shown() {
			this.$store.dispatch("readCompanyAlerts")
					.then(res => {
